...
// Curve
import "../interfaces/curve/ICrvEthPoolLegacy.sol";
// RocketPool
...
contract AsymmetryStrategy is ERC1155Holder, Ownable {
    ...
    function sfrxEthPrice(uint amount) public view returns (uint) {
        ...
        return ICrvEthPoolLegacy(frxEthCrvPoolAddress).get_dy(0, 1, frxAmount);
    }
    ...
    function withdrawWstEth(uint256 _amount) public {
        ...
        ICrvEthPoolLegacy(lidoCrvPool).exchange(1, 0, stEthBal, 0);
    ...
    function addAfEthCrvLiquidity(
        ...
        uint256 poolTokensMinted = ICrvEthPoolLegacy(_pool).add_liquidity(
            _amounts,
            uint256(100000),
            false
        );
    ...
    function withdrawCRVPool(address _pool, uint256 _amount) public {
        ...
        ICrvEthPoolLegacy(afETHPool).remove_liquidity(_amount, min_amounts);
    }
